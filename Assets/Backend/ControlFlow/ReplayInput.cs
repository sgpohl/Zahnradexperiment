...
public class ReplayInput : BaseInput
{
    ...
    private void Update()
    {
        ...
        Vector2 tpos = Camera.main.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, Camera.main.nearClipPlane));
    ...
    public class Data
    {
        ...
        public Point Next()
        {
            ...
            else
            {
                ...
                point.dt = IdleLength;
                point.x = EncodeFloat(block[1], byte.MaxValue - 2, xMin, xMax);
            ...
            point.dt = block[0];
            point.x = EncodeFloat(block[0], 254, xMin, xMax);
            point.y = EncodeFloat(block[1], 254, yMin, yMax);
            //Debug.Log("raw small in: " + block[0].ToString() + "," + block[1].ToString());
            point.MouseDown = true;
            point.MouseChange = false;
        ...
        public void Write(Point p)
        {
            ...
            {
                byte x = (byte)TrimToRange(p.x, byte.MaxValue-2, xMin, xMax);
            ...
            /*
            byte x = TrimToByte(p.x, xMin, xMax);
            byte y = TrimToByte(p.y, yMin, yMax);


            //Debug.Log("cmp: " + p.x.ToString() + "=" + EncodeFloat(x, 254, xMin, xMax).ToString() + " | " + p.y.ToString() + "=" + EncodeFloat(y, 254, yMin, yMax).ToString());
            Debug.Log("cast :" + p.dt.ToString("0.0") + " = " + ((byte)p.dt).ToString());
